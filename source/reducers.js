...
function history(state = initialHistory(), action) {
	...
	const isArrayId = action.nodeId && Array.isArray(action.nodeId);
	const previousIsArrayId = previousAction && previousAction.nodeId && Array.isArray(previousAction.nodeId);
	let isCoalescing = true;
	if (isArrayId && previousIsArrayId && action.nodeId.length === previousAction.nodeId.length) { // New, very tedious check to allow coalescing bezier with multiple values. (Presets are meant to mimic tinkercad API)
		for (let i=0; i<action.nodeId.length; i++) {
			const id = action.nodeId[i];
			const previousId = previousAction.nodeId[i];
			if (id !== previousId) {
				isCoalescing = false;
				break;
			}
...
function normalizedTreeDict(state = initialTreeDict, action) {
	...
	if (action.type === actions.INITIALIZE_PART_TWO) {
		...
		Object.keys(data).forEach(key => {
			const copy = Object.assign({},normalizedTreeDict[key]);
			let value = data[key];
			copy.value = data[key];
		...
		if (Array.isArray(nodeId) && Array.isArray(action.value)) {
			nodeId.forEach( (id,index) => {
				const subValue = action.value[index];
				const subAction = Object.assign({}, action, { nodeId:id, value: subValue });
