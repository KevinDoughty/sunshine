...
function Camera( canvas, opts ) {
  ...
  this.mb.on( "down", ( function() {
    this.mouseDown = true;
    this.resetCheck();
    this.mousePressed(opts.callback);
  } ).bind( this ) );

  this.mp.on( "move", ( function() {
    if( this.mouseDown ) {
      this.mouseDragged(opts.callback);
    }
  } ).bind( this ) );
...
Camera.prototype.mousePressed = function(callback) {
...
Camera.prototype.mouseDragged = function(callback) {
  this.setArcBallVector( this.mp[ 0 ], this.mp[ 1 ] );
...
Camera.prototype.mouseWheel = function(callback) {
  this.position[ 2 ] += this.mw[ 1 ] * 0.01;
  this.mw.flush();
  if (callback)this.opts.callback();
}
